...
df.to_csv("/opt/airflow/data/maroc_cities.csv" , index=False)
...
for index , row in df.iterrows() :
    ...
    if len(latitude_list) == 100:
        ...
        if response.status_code == 200:
            file_name = f"/opt/airflow/data/bronze/Cities_{counter}_Bronze.json"
...
batch_files = glob.glob("/opt/airflow/data/bronze/Cities_*_Bronze.json")
...
with open("/opt/airflow/data/bronze/All_Cities_Bronze.json", "w") as file:
